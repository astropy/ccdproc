...
median_filter.__doc__ = """See `scipy.ndimage.median_filter`.

This function was decorated with ``support_nddata`` and if given a
`CCDData` instance it will use the ``ccd.data`` attribute as input for the
function call to `scipy.ndimage.median_filter`.

For the returned object the following rules apply:
...
TODO: Make this auto-generated by ``support_nddata`` and maybe use
`~astropy.table.Table` to create a rendered table.
"""
