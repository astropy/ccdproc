...
__all__ = ['background_deviation_box', 'background_deviation_filter',
           ...
           'create_deviation', 'flat_correct', 'gain_correct', 'median_filter',
           'rebin', 'sigma_func', 'subtract_bias', 'subtract_dark',
           'subtract_overscan', 'transform_image', 'trim_image', 'wcs_project',
           'Keyword']
...
# Wrapped_functions
median_filter = support_nddata(repack=True, returns=['data'],
                               keeps=['meta', 'unit'],
                               data='input')(ndimage.median_filter)
median_filter.__doc__ = """See also `scipy.ndimage.median_filter`.

=========== ==== ========== ======
  attribute used calculated copied
=========== ==== ========== ======
       data    X          X     --
uncertainty   --         --     --
       mask   --         --     --
...
=========== ==== ========== ======
"""
# The decorator is so good it keeps the module, we want the docstring to
# appear in ccdprocs documentation so we need to overwrite it!
median_filter.__module__ = 'ccdproc'
